...
class PlaceObj(QtWidgets.QDialog):
    ...
    def run_cultivate(self, pos_point, dire_vector):
        ...
        for trans_obj in floor_obj:
            ...
            if (tree_x, tree_y, tree_z) != (0.0, 0.0, 0.0):
                ...
                self.outPrint(hitFacePtr, fn_mesh, dup_obj)
            else:
                continue
    ...
    def outPrint(self, num, fn_mesh, dup_obj):
        #测试阶段的东西 没有用 不影响使用
        ...
        face_normal_rad = [math.atan2(face_normal[2], face_normal[1]), math.atan2(face_normal[2], face_normal[0]),
                           math.atan2(face_normal[1], face_normal[0])]
        face_normal_rot = [face_normal_rad[0] * (180 / math.pi), face_normal_rad[1] * (180 / math.pi),
                           face_normal_rad[2] * (180 / math.pi)]

    def getRotate(self, rot1, rot2):
